//--------------------------------------------------------------------------------------------------
// Vegetation material for MAX (rev 1)
//--------------------------------------------------------------------------------------------------
#ifdef _3DSMAX_
...
//
// Wind parameters placeholder
//
static float WindSpeedX = 0.0f;
static float WindSpeedY = 0.0f;
static float WindFrequency = 0.0f;
static float WindHeightScale = 1000.0f;
...
VS_OUTPUT vs(in VS_INPUT vIn, out float4 position : POSITION)
{
	...
	//
	// Apply wind
	wind(vIn.position.xyz,fElapsedTime,float2(WindSpeedX,WindSpeedY),WindFrequency,WindHeightScale);
	...
	Out.texcoord.xy = vIn.texcoord.xy;
