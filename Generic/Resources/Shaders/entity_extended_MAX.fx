...
texture  EnvMap         	: EnvMap		< string UIName = "Env Map"; string name = "_nomap_black.tga"; string TextureType = "2D"; >;
texture  EnvMapMask        	: EnvMapMask		< string UIName = "Env Mask"; string name = "_nomap_black.tga"; string TextureType = "2D"; >;
...
float    EnvMapFactor			: EnvMapFactor			< string UIName = "Env-Map Factor"; > = 0.0f;
...
float4 ps(VS_OUTPUT vIn) : COLOR
{
	...
	float3 viewNormal = 0.5f + normalize(vIn.viewNormal) * 0.5f;
	viewNormal.y = -viewNormal.y;
	float3 EnvMapTexel = tex2D(EnvMapSampler, viewNormal) * tex2D(EnvMapMaskSampler, vIn.texcoord.xy).r;
